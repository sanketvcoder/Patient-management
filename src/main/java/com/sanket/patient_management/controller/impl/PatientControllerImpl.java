...
public class PatientControllerImpl implements PatientController {
    ...
    @Override
    @DeleteMapping("/{id}")
    public ResponseEntity<String> getPatientById(@PathVariable UUID id) {
        patientService.deleteID(id);;
        return ResponseEntity.status(HttpStatus.NO_CONTENT).body("Patient deleted successfully with id : " + id);
    }
}
